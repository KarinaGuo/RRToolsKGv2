...
#' @examples
#' meta_data <- read_dart_meta(datafile=, nas="-")
#
 
read_meta_info <- function(dart_data, basedir, species, dataset, version="db") {

   metafile <- paste(basedir, species, "/meta/", species, "_", dataset,"_meta.",version,".csv", sep="")
   ...
   m  <- read.table(RR_db_file, header=TRUE, sep=",", quote='"', fill = TRUE, stringsAsFactors=FALSE)
   ...
   if ( fields == 1 ) {
      ...
      varname  <- colnames(meta_ordered)[6]
      analyses <- list()
      analyses[[ varname ]] <- meta_ordered[, 6]
   }


   if ( fields > 1 ) {
      cat(" Adding analysis field to meta data list \n")
      analyses <- as.list(meta_ordered[, 6:(6+fields-1)])
   }
